...
def main():
    ...
    parser.add_argument('--path', default="/newSSD/home/YGY/workDataSets/VoiceBank+DEMAND/")
    ...
    ## train_clean
    make_json(
        os.path.join(prepath, 'clean_trainset_28spk_wav/'),
        'train_clean.json'
    )

    ## train_noisy
    make_json(
        os.path.join(prepath, 'noisy_trainset_28spk_wav/'),
        'train_noisy.json'
    )

    ## valid_clean
    make_json(
         os.path.join(prepath, 'clean_testset_wav/'),
        'valid_clean.json'
    )

    ## valid_noisy
    make_json(
        os.path.join(prepath, 'noisy_testset_wav/'),
        'valid_noisy.json'
    )
